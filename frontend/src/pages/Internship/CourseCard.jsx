...
const CourseCard = ({ course }) => {
  ...
  return (
    <div className=" bg-white p-4 sm:p-6 md:p-6 flex flex-col space-y-4  rounded-2xl hover:shadow-2xl shadow-gray-300 wrap-break-words w-full transform transition duration-200 hover:scale-105">
      <img
        src={course.images[0]}
        alt={course.title}
        className="w-full h-48 md:h-56 lg:h-64 rounded-xl object-cover"
      />

      <h3 className="text-lg sm:text-xl md:text-2xl  font-bold wrap-break-word">{course.title}</h3>
      <p className="text-gray-400 text-sm sm:text-base md:text-lg wrap-break-word">{course.subtitle}</p>
      <p className="text-sm sm:text-base md:text-base">{course.description}</p>
      <ul>
        {course.shortDetail?.map((detail, index) => (
          <li key={index} className="wrap-break-word flex items-start">
            <svg
              width="24px"
              height="24px"
      ...
      <div className="flex flex-col sm:flex-row gap-4 sm:justify-around mt-4">
        <button
          onClick={() => setOpen(true)}
          className="w-full sm:w-auto px-4 py-2.5 rounded-2xl border shadow-2xl shadow-gray-300 transform transition duration-200 hover:scale-105"
        >
          Learn More
        </button>
        <button className="w-full sm:w-auto bg-purple-700 hover:bg-purple-800 text-white px-4 py-2.5 rounded-2xl transform transition duration-200 hover:scale-105">
          Apply Now
        </button>
      </div>
